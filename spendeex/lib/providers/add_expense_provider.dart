...
class AddExpenseProvider with ChangeNotifier {
  ...
  List<ExpenseItem> _items = [ExpenseItem(name: '', amount: 0.0)];
  ...
  void updateExpenseItem(int index, String name, double amount) {
    if (index < _items.length) {
      _items[index] = ExpenseItem(name: name, amount: amount);
      notifyListeners();
    }
  }
  ...
  Future<String?> saveExpense() async {
    ...
    if (_items.isEmpty || _items.every((item) => item.name.isEmpty || item.amount <= 0)) {
    ...
    try {
      final expense = ExpenseModel(
        ...
        items: _items.where((item) => item.name.isNotEmpty && item.amount > 0).toList(),
  ...
  void reset() {
    ...
    _description = '';
    _selectedGroup = null;
    _items = [ExpenseItem(name: '', amount: 0.0)];
